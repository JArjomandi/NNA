...
@section('content')
    <section class="breadcrumbs">
        <div class="container">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <h2>@if(isset($article)) Confirm {{ $article->network_name }} @else Register @endif </h2>
            </div>
    ...
    <section class="inner-page">
        <div class="container">
            ...
            @endif
            @if(count($similar_articles))
                <div class="alert alert-danger" role="alert">
                    <ul>
                        @foreach($similar_articles as $similar_article)
                            <li>
                                {{ $similar_article->network_name }}
                            </li>
                        @endforeach
                    </ul>
                </div>
            @endif
            <form action="@if(isset($article)) {{ route('accept-register-request', encrypt($article->id)) }} @endif" method="POST">
            @csrf
            <div class="mb-3 mt-3">
